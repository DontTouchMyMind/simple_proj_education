...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def throttle(self, target: Union[types.Message, types.CallbackQuery]):
        ...
        try:
            ...
            await self.target_throttled(target, error, dp, key)
            raise CancelHandler()
    ...
    async def target_throttled(target: Union[types.Message, types.CallbackQuery],
        ...
        if throttled.exceeded_count == 2:
            await msg.reply("it was very often!")
            return
        elif throttled.exceeded_count == 3:
            await msg.reply(f"U was banned for throttling at {delta} sek!")
            return
        await asyncio.sleep(delta)

        thr = await dispatcher.check_key(key)
        if thr.exceeded_count == throttled.exceeded_count:
            await msg.reply("ok, we can continue!")

    async def on_process_message(self, message, date):
